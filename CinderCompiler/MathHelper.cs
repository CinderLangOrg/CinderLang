    ...
    public static class MathHelper
    {
        ...
        public static bool IsMathExpression(string value)
        {
            ...
            foreach (char c in value)
            {
                ...
                if (c == '(' && !isInString) depth++;
                else if (c == ')' && !isInString) depth--;

                if (depth == 0 && "+-*/%".Contains(c) && !isInString)
                    return true;
            }
        ...
        public static IType InferExpressionType(string expr,IAstContainerNode scope)
        {
            var tokens = Tokenize(expr);
            tokens.RemoveAll("+-*/()%".Contains);

            List<IType> types = tokens.Select(x => GenerationHelpers.InferTypeFromValue(x,scope)).ToList();

            Console.WriteLine(types.Count);

            TypeKind t = TypeKind.TargetExtTypeKind;
            IType rt = Program.Builder.Int32Type;

            foreach (var item in types)
            {
                Console.WriteLine(item.Kind + " " + t + " " + rt);
