...
namespace CinderLang
{
    ...
    public static class GenerationHelpers
    {
        ...
        public static IValue ParseValue(string value, IType llvmt, IAstContainerNode searchp, bool typeKnown = true,bool loadptr = false)
        {
            ...
            if (IsKeyword(value, "asm")) return BuildASMFromChildren(llvmt, Parser.Iterate(GetBody(value)), []);
            ...
            if (IsVariable(value, searchp)) return ResolveVariable(value, searchp,loadptr,llvmt).Item3;
        ...
        {
            if (!value.Contains('{') || !value.Contains('}')) ErrorManager.Throw(ErrorType.Syntax, $"'{value.Split(' ')[0]}' statement must have a body");
        ...
        public static IValue BuildASMFromChildren(IType t,IAstNode[] children, IValue[] arguments)
        {
            List<string> constlist = new();
            ...
            string[] asm = children.Select(x =>
            {
                ...
                else if (x is ReturnNode ret)
                {
                    if (hasret) ErrorManager.Throw(ErrorType.Syntax, $"Asm statement can only have one return statement");
                    hasret = true;

                    constlist.Insert(0, $"={{{ret.Name.Trim()}}}");
                    return "";
                }
            ...
            if (!hasret && !t.Equals(Program.Builder.VoidType)) ErrorManager.Throw(ErrorType.Syntax, $"Non void asm statements must have a return statement");

            foreach (var item in arguments)
            {
                if (item.TypeOf.Equals(Program.Builder.FloatType) | item.TypeOf.Equals(Program.Builder.DoubleType))
                    constlist.Add("f");
                else constlist.Add("r");
            }

            constlist.Add("~{dirflag},~{fpsr},~{flags}");

            var cstr = string.Join(',', constlist);
            //if (!hasret) cstr = cstr.Insert(0,",");

            return BuildASM(t, string.Join('\n', asm), cstr, arguments);
        }

        public static IValue BuildASM(IType t,string asmstring,string constants, IValue[] arguments)
        {
            var asmt = Program.Builder.CreateFunction(t, []);
            return Program.Builder.BuildCall(asmt, Program.Builder.BuildInlineAsm(asmt, asmstring, constants), arguments);
        }

        static bool IsKeyword(string value,string keyword) => value.Split(' ')[0] == keyword;
        ...
        public static IType InferTypeFromValue(string value, IAstContainerNode scope)
        {
            ...
            if (MathHelper.IsMathExpression(value))
                return MathHelper.InferExpressionType(value,scope);

            if (IsVariable(value, scope))
                return ResolveVariable(value, scope,false,Program.Builder.VoidType).Item1;
        ...
        public static (IType, string, IValue) ResolveVariable(string name,IAstContainerNode node,bool loadptr,IType expectedT)
        {
            if (node.ContextVariables.Any(x=>x.Item2 == name))
            {
                var c = node.ContextVariables.First(x => x.Item2 == name);

                if (!loadptr && !expectedT.Equals(c.Item1)) return c;

                return (c.Item1, c.Item2, Program.Builder.BuildLoad(c.Item1, c.Item3));
            }

            if (node.Parent != null)
                return ResolveVariable(name, node.Parent,loadptr, expectedT);
